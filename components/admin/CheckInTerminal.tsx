...
import { checkInGuest, getGuests } from '../../services/storageService';
...
export const CheckInTerminal: React.FC = () => {
  ...
  const handleSearch = async () => {
      ...
      try {
          ...
          // Find by Hash (Exact) or Name (Includes)
          const guest = allGuests.find(g => 
              g.qrCodeHash.toLowerCase() === searchCode.toLowerCase() || 
              g.name.toLowerCase() === searchCode.toLowerCase() // Simple name search backup
          );
  ...
  const performCheckIn = async () => {
      ...
      setIsLoading(true);
      const success = await checkInGuest(foundGuest.id);
      if (success) {
                ...
                {foundGuest && (
                    <div className="w-full max-w-lg animate-fade-in-up">
                        <div className={`
                            ...
                            ${checkInStatus === 'idle' || checkInStatus === 'error' ? (foundGuest.status === RSVPStatus.CONFIRMED ? 'border-blue-500' : 'border-red-500') : ''}
                        `}>
                        ...
                        <div className="grid grid-cols-2 gap-6 mb-8">
                            <div className="bg-white p-4 rounded shadow-sm">
                                ...
                                <span className="text-2xl font-mono text-stone-800">{foundGuest.confirmedCompanions} / {foundGuest.maxCompanions}</span>
                            </div>
                             <div className="bg-white p-4 rounded shadow-sm">
                                <span className="block text-[10px] uppercase text-stone-400 font-bold">Status RSVP</span>
                                <span className={`text-lg font-bold ${foundGuest.status === RSVPStatus.CONFIRMED ? 'text-green-600' : 'text-red-500'}`}>
                                    {foundGuest.status}
                                </span>
                            </div>
                        </div>
                        ...
                        <div className="flex gap-4 mt-8">
                            ...
                            {checkInStatus === 'idle' && foundGuest.status === RSVPStatus.CONFIRMED && !foundGuest.checkedInAt && (
